...
public:
	...
	*  @see:
	*/
	//HANDLE InitPort(UINT  portNo = 3, UINT  baud = CBR_19200, char  parity = 'N', UINT  databits = 8, UINT  stopsbits = 1, DWORD dwCommEvents = EV_RXCHAR);
	HANDLE InitPort(UINT  portNo, UINT  baud, char  parity, UINT  databits, UINT  stopsbits, DWORD dwCommEvents);
	...
	*  @see:
	*/
	bool WriteData(HANDLE  m_hComm,unsigned char* pData, int length);
